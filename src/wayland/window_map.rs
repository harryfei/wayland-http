    ...
    SD: 'static,
{
    ...
    pub fn insert(&mut self, toplevel: ToplevelSurface<U, R, CID, SD>) {
        let window = Window { toplevel: toplevel };
    ...
    pub fn get_surface(&self, id: u64) -> Option<&WlSurface> {
        self.windows.get(&id).and_then(|w| w.toplevel.get_surface())
    }
    ...
    pub fn get_windows_metadata(&self) -> Vec<WindowMetaData> {
        self.windows
            ...
            .map(|(id, w)| {
                ...
                let title = w.toplevel.get_pending_state().map(|s| s.title);
    ...
    pub fn get_window_metadata(&self, id: u64) -> Option<WindowMetaData> {
        self.windows.get(&id).map(|w| {
            ...
            WindowMetaData { id: id, pid, title }
        })
    }
